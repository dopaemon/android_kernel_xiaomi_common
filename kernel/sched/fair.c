...
static int newidle_balance(struct rq *this_rq, struct rq_flags *rf)
{
	...
	/*
	 * Decide whether to attempt a newidle balance early. We need 'sd'
	 * to update next_balance if we bail out, so take RCU and fetch it.
	 */
	rcu_read_lock();
	sd = rcu_dereference_check_sched_domain(this_rq->sd);

#ifdef CONFIG_CACULE_SCHED
	can_newidle = !READ_ONCE(this_rq->rd->overload) ||
